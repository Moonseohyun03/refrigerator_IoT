...
def get_refrigerator_number():
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, "r") as f:
            refrigerator_number = f.read().strip()
            if refrigerator_number:
                print(f"저장된 냉장고번호 사용: {refrigerator_number}")
                return refrigerator_number

    refrigerator_number = input("냉장고번호 입력: ").strip()
    with open(CONFIG_FILE, "w") as f:
        f.write(refrigerator_number)
    return refrigerator_number
...
if __name__ == "__main__":
    refrigerator_number = get_refrigerator_number()
